...
class SetupSSH {
  ...
  writeAuthorizedKeys(pubkey) {
    ...
    const content = keys.map(k => `${k} # Added by setup-ssh at ${timestamp}`).join('\n') + '\n';
...
`.trim();
   ...
   */
  async windowsSetup(sshPort, _allowUsers) {
    ...
    const settings = [`Port ${sshPort}`, 'PubkeyAuthentication yes', 'PasswordAuthentication no'];
