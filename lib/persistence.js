  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        logger.debug(`Failed to read error response: ${readErr.message}`);
      }
    ...
    const msg =
      err?.name === 'AbortError' ? `Timeout after ${config.timeouts.httpRequest}ms` : err?.message || String(err);
  ...
  try {
    ...
    const result = spawnSync(
      ...
      [
        ...
        ntfyUrl,
        '-H',
        'X-Title: SSH Tunnel Ready',
        '-H',
        'X-Priority: 3',
        '-H',
        'X-Tags: ssh,tunnel,ci',
        '-H',
